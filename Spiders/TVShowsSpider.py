...
class TvShowsSpider(scrapy.Spider):
    name = "tvshows_spider"  # Nombre del spider
    user_agent = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, como Gecko) Chrome/109.0.0.0 Safari/537.36'  # Agente de usuario para las solicitudes HTTP
    ...
    def __init__(self, year):
        self.year = year  # Año para filtrar las series de televisión
    ...
    def parse(self, response):
        ...
        for item in response.css('div.c-finderProductCard'):
            # Extraer la URL de la página del detalle de la serie de televisión
            url = item.css('a.c-finderProductCard_container::attr(href)').get()
            if url:
                # Seguir la URL y procesar el detalle de la serie de televisión con el método parse_item
    ...
    def parse_item(self, response):
        ...
        try:
            ...
            duration_selector = 'div.c-productionDetailsTv_details span.g-text-bold:contains("Number of seasons:") + span::text'
            ...
            if user_score != "tbd" and metascore != "tbd":
                # Crear un diccionario con la información de la serie de televisión
                tvshow_data = {
                    'title': title,
                ...
                # Agregar la información de la serie de televisión a la lista de datos
